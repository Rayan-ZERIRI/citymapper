...
class MainWindow(QMainWindow):
    ...
    def connect_DB(self):
        self.conn = psycopg2.connect(database="", user="", host="", password="")
    ...
    def mouseClick(self, lat, lng):
        ...
        self.cursor.execute(""f" WITH mytable (distance, name) AS (SELECT ( ABS(latitude-{lat}) + ABS(longitude-{lng}) ), nom_long FROM metros) SELECT A.name FROM mytable as A WHERE A.distance <=  (SELECT min(B.distance) FROM mytable as B)  """)
        self.conn.commit()
        rows = self.cursor.fetchall()
